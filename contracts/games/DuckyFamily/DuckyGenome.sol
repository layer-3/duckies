...
library DuckyGenome {
    ...
    function generateAndSetGenes(
        ...
        if (collectionId == ducklingCollectionId) {
            IDuckyFamily.Rarities rarity = IDuckyFamily.Rarities(
                genome.getGene(rarityGeneIdx)
            );

            if (rarity == IDuckyFamily.Rarities.Common) {
                genome = genome.setGene(
                    uint8(IDuckyFamily.GenerativeGenes.Body),
                    0
                );
                genome = genome.setGene(
                    uint8(IDuckyFamily.GenerativeGenes.Head),
                    0
                );
     ...
     */
    function requireGenomesSatisfyMelding(
        ...
        IDuckyFamily.Rarities rarity = IDuckyFamily.Rarities(
            genomes[0].getGene(rarityGeneIdx)
        );
        ...
        if (rarity == IDuckyFamily.Rarities.Common) {
            ...
            // Rare, Epic
            if (
                rarity == IDuckyFamily.Rarities.Rare ||
                rarity == IDuckyFamily.Rarities.Epic
            ) {
                if (
                ...
                if (
                    ...
                    genomes[0].getGene(collectionGeneIdx) ==
                    zombeakCollectionId ||
    ...
    function meldGenes(
        ...
        if (
            ...
            return
                maxPresentGeneValue == maxGeneValue
                    ? maxGeneValue
                    : maxPresentGeneValue + 1;
        }

        // gene inheritance
        uint8 inheritanceIdx = Utils.randomWeightedNumber(
            inheritanceChances,
            bitSlice
        );
    ...
    function calcConfigPeculiarity(
        ...
        for (uint8 i = 0; i < genesNum; i++) {
            if (
                getDistributionType(geneDistrTypes, i) ==
                IDuckyFamily.GeneDistributionTypes.Uneven
            ) {
