...
contract DuckyFamilyV1 is IDuckyFamily, AccessControl, Seeding {
    ...
    function _useVoucher(Voucher memory voucher) internal {
        ...
        if (voucher.action == uint8(VoucherActions.MintPack)) {
            ...
            MeldParams memory meldParams = abi.decode(
                voucher.encodedParams,
                (MeldParams)
            );
            ...
            // meldParams checks
            if (
                meldParams.owner == address(0) ||
                meldParams.tokenIds.length != FLOCK_SIZE
            ) revert InvalidMeldParams(meldParams);

            _meldOf(
                meldParams.owner,
                meldParams.tokenIds,
                meldParams.isTransferable
            );
        } else {
     ...
     */
    function getMeldPrices()
        external
        view
        returns (uint256[RARITIES_NUM] memory)
    {
        return meldPrices;
    }
     ...
     */
    function getCollectionsGeneValues()
        external
        view
     ...
     */
    function getCollectionsGeneDistributionTypes()
        external
        view
    ...
    function _mintPackTo(
        ...
        for (uint256 i = 0; i < amount; i++) {
            tokenGenomes[i] = _generateGenome(
                ducklingCollectionId,
                _randomSeed()
            ).setFlag(Genome.FLAG_TRANSFERABLE, isTransferable);
        }
     ...
     */
    function _generateGenome(
        uint8 collectionId,
        bytes32 seed
    ) internal view returns (uint256) {
        if (
            collectionId != ducklingCollectionId &&
            collectionId != zombeakCollectionId
        ) {
        ...
        genome = genome.setGene(
            rarityGeneIdx,
            Utils.randomWeightedNumber(rarityChances, bitSlice)
        );
        ...
        genome = genome.setGene(
            Genome.MAGIC_NUMBER_GENE_IDX,
            Genome.BASE_MAGIC_NUMBER
        );
    ...
    function _generateMythicGenome(
        ...
        uint16 pivotalUniqId = uint16(
            (uint64(flockPeculiarity) * maxUniqId) / maxSumPeculiarity
        ); // multiply and then divide to avoid float numbers
        ...
        uint16 uniqId = leftEndUniqId +
            uint16(Utils.randomNumber(bitSlice, uniqIdSegmentLength));
    ...
    function meldFlock(uint256[] calldata meldingTokenIds) external {
        ...
        uint256 meldPrice = meldPrices[
            ducklingsContract.getGenome(meldingTokenIds[0]).getGene(
                rarityGeneIdx
            )
        ];
        duckiesContract.transferFrom(
    ...
    function _meldOf(
        ...
        if (meldingTokenIds.length != FLOCK_SIZE)
        ...
        uint256 meldedGenome = _meldGenomes(meldingGenomes, _randomSeed())
            .setFlag(Genome.FLAG_TRANSFERABLE, isTransferable);
    ...
    function _meldGenomes(
        ...
        if (collectionId == ducklingCollectionId) {
            if (
                DuckyGenome.isCollectionMutating(
                    rarity,
                    collectionMutationChances,
                    bitSlice
                )
            ) {
                uint256 zombeakGenome = _generateGenome(
                    zombeakCollectionId,
                    updatedSeed
                );
        ...
        // increase rarity
        meldedGenome = meldedGenome.setGene(
            rarityGeneIdx,
            genomes[0].getGene(rarityGeneIdx) + 1
        );
        ...
        if (collectionId == ducklingCollectionId) {
            ...
            if (rarity == Rarities.Common) {
                meldedGenome = DuckyGenome.generateAndSetGene(
                    ...
                    geneValuesNum[
                        uint8(GenerativeGenes.Body) - generativeGenesOffset
                    ],
                ...
                meldedGenome = DuckyGenome.generateAndSetGene(
                    ...
                    geneValuesNum[
                        uint8(GenerativeGenes.Head) - generativeGenesOffset
                    ],
                    GeneDistributionTypes.Uneven,
                    bitSlice
                );
